...
class MyNeuralNetClassifier(NeuralNet, BaseEstimator):
    ...
    def create_clf(self, model_params, task_name):
        ...
        if self.bert_weights is not None or hyper_params.CONDUCT_TRANSFER_LEARNING:
          ...
          if self.bert_weights != None:
            if os.path.exists(self.bert_weights + '_original'):
              pretrain_state_dict = torch.load(self.bert_weights + '_original', map_location='cuda')
            else:
              pretrain_state_dict = torch.load(self.bert_weights, map_location='cuda')
              torch.save(pretrain_state_dict, os.path.join(os.path.abspath(os.curdir), f"loading_eicu_weights"))
    ...
    def train_step_single(self, batch, epoch_num = 0, prev_batch = None, is_train=True, **fit_params):
