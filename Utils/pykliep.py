#From https://github.com/srome/pykliep/blob/master/pykliep.py
...
class DensityRatioEstimator:
    ...
    def __init__(self, max_iter=5000, num_params=[.1,.2], epsilon=1e-4, cv=3, sigmas=[200,400,500,600,750,1000], random_state=None, verbose=0):
        """ 
    ...
    def _find_alpha(self, alpha_0, X_train, X_test, num_parameters, sigma, epsilon):
        ...
        for k in range(self.max_iter):
            out += epsilon*np.dot(np.transpose(A),1./(np.dot(A,out) + self.epsilon))
            out += b*(((1-np.dot(np.transpose(b),out))/(np.dot(np.transpose(b),b) + self.epsilon)))
